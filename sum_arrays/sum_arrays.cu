...
__global__ void sumArraysKernel(float *a, float *b, float *res)
{
...
int main(int argc, char **argv)
{
    ...
    sumArraysKernel<<<grid, block>>>(a_d, b_d, res_d);
    printf("Execution configuration<<<%d,%d>>>\n", grid.x, block.x);

    CHECKCUDAERR(cudaMemcpy(res_from_gpu_h, res_d, nByte, cudaMemcpyDeviceToHost));
